...
__webpack_require__(/*! ./vendor/scripts/timber.js */ "./src/vendor/scripts/timber.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.avalanche.js */ "./src/vendor/scripts/jquery.tm.avalanche.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.counter.js */ "./src/vendor/scripts/jquery.tm.counter.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.equalize.js */ "./src/vendor/scripts/jquery.tm.equalize.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.horizon.js */ "./src/vendor/scripts/jquery.tm.horizon.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.retinize.js */ "./src/vendor/scripts/jquery.tm.retinize.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.snowbridge.js */ "./src/vendor/scripts/jquery.tm.snowbridge.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.summit.js */ "./src/vendor/scripts/jquery.tm.summit.js");

__webpack_require__(/*! ./vendor/scripts/jquery.tm.swipe.js */ "./src/vendor/scripts/jquery.tm.swipe.js");
...
/***/ "./src/vendor/scripts/jquery.tm.avalanche.js":
/*!***************************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.avalanche.js ***!
  \***************************************************/
/*! no static exports found */
...
(function ($, document, window, undefined) {
  ...
  var AvalancheSlider = function AvalancheSlider(element, options) {
    // Settings
    var settings = $.extend({}, $.fn.avalancheSlider.tmsOpts, options); // Variables used throughout
    ...
    var slidesArray = [];
    var origAnimation; // Set slide ids
    ...
    if (!settings.lazyLoad) active.css({
      opacity: 0
    }); // Check scale under data settings

    settings.scaleUnder = slider.data('scale-under') ? slider.data('scale-under') : settings.scaleUnder; // Check min acceptable height for slider

    settings.scaleMinHeight = slider.data('scale-min-height') ? slider.data('scale-min-height') : settings.scaleMinHeight; // Check animation data settings

    settings.animation = slider.data('animation') ? slider.data('animation') : settings.animation;
    origAnimation = settings.animation; // Check easing data settings

    settings.easing = slider.data('easing') ? slider.data('easing') : settings.easing; // Check speed data settings

    settings.speed = slider.data('speed') ? slider.data('speed') : settings.speed; // Check carousel data settings
    ...
    settings.carouselVisible = settings.carouselVisible > slides.children().length ? slides.children().length : settings.carouselVisible; // Check auto advance data settings
    ...
    settings.pauseOnHover = slider.is('[data-pause-on-hover="false"]') ? false : settings.pauseOnHover; // Check nav data settings
    ...
    settings.navShowOnHover = slider.is('[data-nav-show-on-hover="false"]') ? false : settings.navShowOnHover; // Check whether to add global nav dark class to header
    ...
    settings.parallaxFadeOut = slider.is('[data-parallax-fade-out]') ? true : settings.parallaxFadeOut; // Check caption scaling option
    ...
    if (settings.carouselScaleHeight) slider.addClass('tms-scalable-height'); // Check video API setting for mobile
    ...
    if (settings.navShowOnHover) slider.addClass('show-on-hover'); // Add arrow nav

    if (settings.navArrows && slides.children().length > 1) {
      // Build
      var prev = $('<a href="#" />').attr('id', 'tms-prev').addClass('tms-arrow-nav').appendTo(slider);
      var next = $('<a href="#" />').attr('id', 'tms-next').addClass('tms-arrow-nav').appendTo(slider); // Actions
      ...
      if (settings.lazyLoad) slider.find('.tms-arrow-nav').css({
        display: 'block'
      });
    } // Add pagination nav
    ...
    if (settings.navPagination && slides.children().length > 1) {
      ...
      var pageNav = settings.paginationType === 'bullets' ? slider.find('.tms-bullet-nav') : slider.find('.tms-thumb-nav'); // Actions
      ...
      pagination.find('.tms-bullet-nav:first-child').addClass('active'); // Show pagination

      if (settings.lazyLoad) pagination.css({
        display: 'block'
      });
    } // Add keyboard nav
    ...
    slider.swipeIt({
      draggable: false,
      onSwipeMove: function onSwipeMove(position) {// translateX values here
      },
      onSwipeEnd: function onSwipeEnd(swipe) {
        if (swipe === 'left') {
          obj.prevSlide();
        } else {
          obj.nextSlide();
    ...
    $(window).on('resize', function () {
      ...
      if (slider.data('first-load')) {
        ...
        slider.data('refW', refW).data('refH', refH).data('carousel-height', refH);
      } // Call resize
      ...
      positionCarousel(slider, settings); // Callback
    ...
    slider.data('setup', false).data('loaded', false).data('first-load', true).data('transitioning', false).data('refW', refW).data('refH', refH).data('carousel-height', refH); // Set caption data attributes

    setCaptionAttrs(slider); // Size slider container

    positionCarousel(slider, settings); // Setup callback
    ...
    */
    ...
    var animateSlideTimer = null; // Init Slideshow

    obj.initSlideshow = function () {
      // If timer already exists prevent
      // it from reintializing
      if (sliderTimer || slides.children().length < 2) return false; // Change default as slideshow
      // is now active

      settings.autoAdvance = true; // Add mouse events
      ...
      if (settings.showProgressBar) progressBar = $('<div>').addClass('tms-progress-bar').appendTo(slider); // Call start

      obj.startSlideshow(); // Animate progress bar initially

      animateProgress(settings.interval); // Set up time ref

      startTime = new Date().getTime();
      baseInterval = settings.interval; // Ss start callback
    ...
    obj.startSlideshow = function (interval) {
      ...
      sliderTimer = setInterval(function () {
        ...
        if (interval !== settings.interval) {
          ...
          baseInterval = settings.interval; // Start slideshow again
        ...
        startTime = new Date().getTime();
        if (typeof progressBar !== 'undefined') progressBar.css({
          transition: 'none',
          width: '0px'
        });
    ...
    obj.pauseSlideshow = function () {
      ...
      if (paused || !sliderTimer) return false; // Clear timer

      clearInterval(sliderTimer); // Get time when paused

      endTime = new Date().getTime(); // Calculate time left of cycle

      timeDiff = baseInterval - (endTime - startTime) < 50 ? 50 : baseInterval - (endTime - startTime);
      baseInterval = baseInterval === 0 ? settings.interval : timeDiff; // Stop progress bar animation

      if (typeof progressBar !== 'undefined') progressBar.stop().css({
        width: progressBar.width() + 'px'
      });
      paused = true; // Ss paused callback
    ...
    obj.resumeSlideshow = function () {
      ...
      if (!paused) return false; // Clear timer

      clearInterval(sliderTimer); // No longer paused

      slider.data('ssPaused', false); // Update start time

      startTime = new Date().getTime(); // Start slideshow with time left in cycle
    ...
    obj.resetSlideshow = function () {
      ...
      clearInterval(sliderTimer); // Set progress to nill
      ...
      animateProgress(settings.interval); // Reset all time variables to origin
    ...
    obj.endSlideshow = function () {
      // Check that timer exists
      if (!sliderTimer) return false;
      clearInterval(sliderTimer); //Remove timer ref

      sliderTimer = '';
      paused = false; // Remove mouse events
      ...
      if (typeof progressBar !== 'undefined') progressBar.remove(); // Ss end callback

      if (settings.onSlideshowEnd) settings.onSlideshowEnd();
    }; // Animate function for bar


    var animateProgress = function animateProgress(interval) {
    ...
    */
    ...
    obj.nextSlide = function () {
      ...
      var index; // Update active slide

      active = slider.find('li.active');
      var nextSlide = slidesArray[$.inArray(active.attr('id'), slidesArray) + 1];
      index = $.inArray(nextSlide, slidesArray) + 1 === 0 ? 1 : $.inArray(nextSlide, slidesArray) + 1; // Crousel true, more than 1 visible slide, set right limit
    ...
    obj.prevSlide = function () {
      ...
      index = $.inArray(prevSlide, slidesArray) + 1 === 0 ? slides.children().length : $.inArray(prevSlide, slidesArray) + 1; // Crousel true, more than 1 visible slide, set left limit
    ...
    */
    ...
    obj.slideTo = function (index, direction) {
      ...
      // slider now in use
      slider.data('first-load', false); // If called directly updated active ref

      if (!direction) active = slider.find('li.active');
      if (slider.data('transitioning') || index === $.inArray(active.attr('id'), slidesArray) + 1) return false; // Update pagination

      updatePagination(slider, index); // Get slider width in case
      // it has changed since previous animation	

      var sliderW = slider.width();
      var sliderH = slider.height(); // Get target slide & swap z-index

      var target = slider.find('#tms-slide-' + index);
      target.addClass('target').css({
        zIndex: 2,
        opacity: tx === 0 ? 0 : 1
      });
      active.css({
      ...
      settings.animation = target.data('animation') ? target.data('animation') : origAnimation; // Check slide animation type
      ...
      var scale = settings.animation === 'scaleIn' ? 1 - settings.scaleFactor : settings.animation === 'scaleOut' ? 1 + settings.scaleFactor : 1; // Check whether dark nav class should be added
      ...
      positionNextCaption(target, settings); // Redimension slider and target slide
      // before brought into view

      if (target.children('img').data('loaded') || !target.children('img').length) redimensionSlider(target, settings);
      positionCarousel(slider, settings, target); // If undefined it will move next

      if (!direction) direction = 'next'; // Verify direction
      ...
      var visibleSlides = carouselBreakPoints(slider, settings); // Move to slide
      ...
      if (tSupport) {
        if (settings.carousel) {
          ...
          target.css({
            opacity: 0,
            transition: 'none',
            transform: 'translate3d(' + tx * direction + 'px' + ',' + ty * direction + 'px' + ', 0) scale3d(' + scale + ', ' + scale + ', ' + scale + ')'
          });
        ...
        if (settings.carousel) {
          ...
          slides.animate({
            opacity: 1,
            left: -(tx / visibleSlides) * direction + 'px'
          }, settings.speed, settings.easingFallback, function () {
          ...
          active.animate({
            left: -sliderW * direction + 'px'
          }, settings.speed, settings.easingFallback);
        }
      }
    }; // Preload slides
  ...
  */
  ...
  function setCaptionAttrs(slider) {
    slider.find('.tms-caption').each(function () {
      ...
      if (caption.is('[data-no-scale]')) caption.addClass('no-scale');else caption.addClass('scale'); // Add perspective class to caption parent

      caption.parent().addClass('tms-perspective'); // Check for presets
  ...
  */
  ...
  function preload(slider, settings, obj) {
    ...
    slide.each(function () {
      ...
      var caption = currentSlide.children('.tms-caption').length; // Prepare captions of current slide and
      // set their initial animation position

      positionNextCaption(currentSlide, settings); // Add preloader based on 
      // preloading option
      ...
      if (video) {
        ...
        if (mobile && currentSlide.is('[data-video-bkg]')) {
          currentSlide.children('video').css({
            display: 'none'
          });
        }
      ...
      if (!totalSlideImgs) {
        ...
        // each slide
        currentSlide.find('img').each(function (i, src) {
          // Check if retina
          src = $(this).data('src');
          ...
          if (window.isRetinaDevice() && settings.retinaSupport || window.isRetinaDevice() && $(this).is('[data-retina]')) {
            if (!mobile && !settings.retinaSupportMobile || mobile && settings.retinaSupportMobile) {
              if (!src.match(/\.(svg)/i)) {
                if (!$(this).data('retina')) src = $(this).data('src').replace(imgExt, settings.retinaSuffix + imgExt);else src = $(this).data('retina');
              }
            }
          } // Overaly
          ...
          if (currentSlide.data('overlay-bkg-color') && !currentSlide.find('.tms-overlay').length) {
            ...
            var opacity = currentSlide.data('overlay-bkg-opacity') ? currentSlide.data('overlay-bkg-opacity') : 0.5; // Convert hex to rgba

            var r = parseInt(bkg.substring(0, 2), 16);
            var g = parseInt(bkg.substring(2, 4), 16);
            var b = parseInt(bkg.substring(4, 6), 16);
            var rgba = 'rgba( ' + r + ',' + g + ',' + b + ',' + opacity / 1 + ' )'; // Set color

            var overlay = $('<div class="tms-overlay" />').css({
          ...
          $(this).css({
            opacity: 0
          }).attr('src', src).one('load', function () {
            currentSlide.css({
              transition: 'none',
              opacity: 0
            });
            currentSlide.find(overlay).css({
              opacity: 1
            });
            $(this).data('loaded', true).css({
              opacity: 1
            });
            i++;
  ...
  */
  ...
  function loaded(slide, totalSlides, settings, obj) {
    ...
    positionCarousel(slider, settings); // Initial opacity
    ...
    if (!settings.lazyLoad) {
      if (totalSlides === slides.children().length) {
        // Delete loader
        if (slider.find('.tm-loader').length) slider.find('.tm-loader').remove(); // Show captions of active

        active.find('.tms-caption').show(); // If visible slides is greater than 1 show all

        if (settings.carouselVisible > 1) slider.find('.tms-caption').show(); // Animate slides 
        // or slides
        ...
        slider.find('.tms-arrow-nav, .tms-pagination').css({
          display: 'block'
        });
      }
    } else {
      // Delete loader
      if (slide.find('.tm-loader').length) slide.find('.tm-loader').remove(); // Show captions if active or visible slides
      // is greater than 1

      if (slide.is(active) || settings.carouselVisible > 1) slide.find('.tms-caption').show();
      if (!slide.hasClass('active')) return false; // Animate slide
  ...
  */
  ...
  var requestScroll = function requestScroll(slider, settings) {
    var win = $(window);
  ...
  var updateParallaxElement = function updateParallaxElement(slider, settings) {
    var win = $(window);
    ...
    if (!mobile && tSupport && isElementVisible(slider.parent())) {
      slider.css({
        transform: 'translate3d( 0, ' + -distance + 'px' + ', 0) scale3d( ' + scale + ',' + scale + ', 1 )',
  ...
  */
  ...
  var animate = function animate(selector, opacity, transX, transY, transZ, rotateX, rotateY, rotateZ, scale, transOrigX, transOrigY, duration, delay, easing, settings) {
    ...
    if (selector.hasClass('active') || selector.hasClass('target') || selector.hasClass('tms-slides')) {
      // Slider is now animating
      if (!slider.data('animate-first-slide')) slider.data('transitioning', true); // Slider before callback
    ...
    var attrs = {};
    attrs.transform = 'translate3d(' + transX + 'px' + ', ' + transY + 'px' + ', ' + transZ + 'px' + ') ' + 'rotateX(' + rotateX + 'deg) ' + 'rotateY(' + rotateY + 'deg) ' + 'rotateZ(' + rotateZ + 'deg) ' + 'scale3d(' + scale + ', ' + scale + ', ' + scale + ')';
    ...
    selector.css(attrs).on(transitionEnd, function (event) {
      // Prevent bubbling
      event.stopPropagation(); // Remove listener

      $(this).off(transitionEnd); // Only call animation complete if currently animation
  ...
  var animateComplete = function animateComplete(slider, settings) {
    // Slider animation done
    slider.data('transitioning', false); // Slider after callback

    if (settings.onSlideAfter) settings.onSlideAfter(); // Get references to active and
    // target slide

    var active = slider.find('li.active');
    var target = slider.find('li.target'); // Pause video of active slide

    media(active, 'pause', settings); // Update active slide reference

    if (settings.carouselVisible === 1) active.find('.tms-caption').not('.no-transition').css({
      display: 'none',
      visibility: 'hidden'
    });
    if (!settings.carousel) active.css({
      visibility: 'hidden'
    });
    active.removeClass('active');
    target.removeClass('target').addClass('active');
    active = target; // Play video of new active slide

    media(active, 'play', settings); // Prevent captions from being animated if carousel visible
    // slides is great that 1

    if (settings.carouselVisible > 1) return false; // Animate caption

    animateCaption(active, settings);
  };

  var animateCaption = function animateCaption(active, settings) {
    // Iterate over captions in active 
    // slide and reset attributes to original and 
    // call animate
    active.find('.tms-caption').not('.no-transition').show().each(function () {
      var caption = $(this);
  ...
  */
  ...
  var updatePagination = function updatePagination(slider, index) {
  ...
  */
  ...
  var carouselBreakPoints = function carouselBreakPoints(slider, settings) {
    var visibleSlides;
  ...
  */
  ...
  var positionCarousel = function positionCarousel(slider, settings, target) {
    ...
    var tallestSlide = 0;
    var animateCarouselTimer = null; // Recalculate slide container
    ...
    pagination.children().hide().slice(0, slides.children().length - visibleSlides + 1).show(); // Swap active slide
    ...
    if (tSupport) {
      if (!slider.data('transitioning')) slides.css({
        width: width * totalSlides + 'px',
        transition: 'none',
        transform: 'translate3d(' + -(width * i) + 'px' + ', 0, 0)'
      });
  ...
  */
  ...
  var redimensionSlider = function redimensionSlider(slide, settings) {
    ...
    var forceFit = settings.forceFit || slide.is('[data-force-fit]'); // Set new slider with and height
    // to be used for captions

    slider.data('newSW', newSliderW);
    slider.data('newSH', newSliderH); // Size wrapper
    ...
    if (settings.carousel) {
      ...
      if (settings.parallax) slider.parent().css({
    ...
    if (!slider.data('setup') || settings.carousel && settings.carouselVisible > 1) return false; // Check slide type

    var type = slide.is('[data-image]') ? 'image' : 'video'; // Check whether video is background video

    var bkgVideo = slide.is('[data-video-bkg]') ? true : false; // Check if a ratio is set for regular video

    var videoRatio = slide.is('[data-video-ratio]') ? parseFloat(slide.data('video-ratio')) : 1.778; // Size slide image or video

    slide.find('img, iframe, video, .mejs-container').not('.tms-caption img, .tms-caption iframe, .tms-caption video').each(function () {
      var mediaW;
      var mediaH;
      var mediaR; // Width & height references
      ...
      mediaR = mediaW / mediaH; // Calculate size if slider is set to fullscreen, 
  ...
  */
  ...
  var redimensionCaptions = function redimensionCaptions(slide, settings) {
    var slider = slide.closest('.tm-slider-container');
    ...
    slide.find('.tms-caption').each(function () {
      if (!$(this).is('[data-no-scale]')) {
        // Get references
        var caption = $(this); // Position values

        var l = caption.data('x');
        var t = caption.data('y'); // Pixel values
        ...
        var ryof = caption.is('[data-offsety]') ? parseFloat(caption.data('offsety')) : 0; // Calculate new offsets

        var xof = newSliderW * (rxof / refW) < 0 ? newSliderW * (rxof / refW) : newSliderW * (rxof / refW) > rxof ? rxof : newSliderW * (rxof / refW);
        var yof = newSliderH * (ryof / refH) < 0 ? newSliderW * (ryof / refW) : newSliderW * (ryof / refW) > ryof ? ryof : newSliderH * (ryof / refH); // Adjust y offset if fullwidth is in use as a fixed
        // height might be set

        if (settings.fullwidth && yof < ryof && yof < 0) yof = ryof; // Calculate caption width if iframe exists
  ...
  */
  ...
  var positionNextCaption = function positionNextCaption(slide, settings) {
    ...
    slide.find('.tms-caption').not('.no-transition').each(function () {
      ...
      if (tSupport) {
        caption.css({
          transition: 'none',
          transform: 'translate3d(' + tx + 'px' + ', ' + ty + 'px' + ', ' + tz + 'px' + ' )' + 'rotateX(' + rx + 'deg) ' + 'rotateY(' + ry + 'deg) ' + 'rotateZ(' + rz + 'deg) ' + 'scale3d(' + s + ', ' + s + ', ' + s + ')',
          opacity: o
        });
      } else {
        caption.css({
  ...
  */
  ...
  var media = function media(slide, status, settings) {
  ...
  */
  ...
  var ytPlayer = {}; // Flags for whether 
  // scripts have been added to page

  var vimeo = false;
  var youtube = false;

  var addVideoEvents = function addVideoEvents(video, settings) {
    if (!settings.useVideoAPI || settings.carouselVisible > 1) return false;
    var url = video.attr('src'); // Set a unique id.

    var currentDate = new Date();
    video.attr('id', 'video-' + currentDate.getTime()); // Vimeo

    if (typeof url != 'undefined' && url.indexOf('vimeo') > -1) {
      // Add id to end of url
      video.attr('src', video.attr('src') + '&player_id=' + video.attr('id')).addClass('vimeo'); // Add script once
      ...
      video.on('load', function () {
        var id = '#' + $(this).attr('id');
        var vp = $f($(id)[0]);
        var firstSlide = $(this).closest('li').is(':first-child') ? true : false; // Add listeners

        vp.addEvent('ready', function () {
          // Play if first
          if (firstSlide && settings.autoPlay) vp.api('play'); // Replay
      ...
      // Add class
      video.addClass('youtube'); // Add script once
      ...
      var addYTEvents = function addYTEvents() {
  ...
  */
  ...
  var transitionEnd = 'webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend'; // Easing Array
  ...
  $.fn.avalancheSlider = function (options) {
    ...
    return this.each(function () {
      ...
      if (element.data('avalancheSlider')) return; // Pass options

      var avalanche = new AvalancheSlider(this, options); // Store plugin object in this element's data
  ...
  $.fn.avalancheSlider.tmsOpts = {
    ...
    scaleFactor: 0.2,
    // Scale factor: used to determine scale factor if animation is set to scale
    parallax: false,
    // Parallax: boolean
    easing: 'easeInOutQuint',
    // Easing type: string, see easingArray
    easingFallback: 'easeInOutQuint',
    // Easing fallback: for older browser that do not support custom easing
    speed: 700,
    // Animation speed: milliseconds 
    parallaxSpeed: 0.2,
    // Parallax speed: decimal - 0.1-0.5 
    parallaxScaleOut: false,
    // Parallax scale out: boolean - if slider should scale out of view
    parallaxFadeOut: false,
    // Parallax fade out:boolean - if slider should fade out of view
    // Navigation
    navArrows: true,
    // Arrow nav: boolean
    navPagination: true,
    // Pagination nav: boolean
    navShowOnHover: true,
    // Show on hover: boolean
    paginationType: 'bullets',
    // Pagination type: string - v.1 bullets only
    navKeyboard: true,
    // Keyboard nav: boolean
    // Scaling
    forceFit: false,
    // Force fits all slide images to slide, use data-force-fit for individual images
    fullwidth: false,
    // Full width: boolean
    fullscreen: false,
    // Fullscreen slider: boolean
    fsUseHeightOf: '',
    // Whether in fullscreen mode the slider should use window height or parent height as reference. Useful when slider is used as background slider.
    externalPadding: 0,
    // External Padding: padding of wrapping container, use data-external-padding for individual sliders
    scaleUnder: 1140,
    // Width under which slider should scale if fullwidth is true							
    scaleMinHeight: 214,
    captionScaling: true,
    // Scale captions: boolean
    // Carousel
    carousel: false,
    // Carousel Slider: boolean
    carouselVisible: 1,
    // Carousel visible slides: integer
    carouselScaleHeight: false,
    // Slideshow
    autoAdvance: true,
    // Slideshow: boolean
    showProgressBar: true,
    // Progress: boolean, wheter slider should show progress bar
    interval: 6000,
    // Interval: milliseconds
    pauseOnHover: true,
    //Video
    useVideoAPI: true,
    // Use YouTube/Vimeo/HTML5 APIs: boolean
    autoPlay: true,
    // Play if active slide: boolean
    replayOnEnd: true,
    // On video end reply: boolean
    respectRatio: true,
    // Scale video based on video ratio: boolean
    muteBkgVideo: false,
    // Whether background video should be muted
    // Preload
    lazyLoad: true,
    // Load progressively: boolean
    // Retina Support
    retinaSupport: true,
    // Check for retina displays and serve retina image, affects all images within a slide
    // Use data-retina for individual swaps
    retinaSupportMobile: false,
    // Whether swap should occur on mobile devices
    retinaSuffix: '@2x',
    // Retina image suffix
    // Callback
    onSetup: null,
    ...
    onLoadEnd: null,
    // Callback: after all slides have loaded
    onSlideBefore: null,
    // Callback: before slide animation
    onSlideAfter: null,
    // Callback: after slide animation
    onSlideshowStart: null,
    // Callback: on slideshow init
    onSlideshowPause: null,
    // Callback: on slideshow pause
...
/***/ "./src/vendor/scripts/jquery.tm.counter.js":
/*!*************************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.counter.js ***!
  \*************************************************/
/*! no static exports found */
...
(function ($) {
  ...
  var Counter = function Counter(element, options) {
    // Settings
    var settings = $.extend({}, $.fn.counter.tmcOpts, options); // Object & jQuery el

    var el = $(element);
    var obj = this; // Timer

    var countTimer; // Start counter

    obj.startCounter = function () {
      // Ensure it's cleared for
      // recurring events 
      obj.clearCounter(); // Check if individual data attributes are set otherwise use
      // default parameters
      ...
      var countInterval = el.data('count-interval') ? parseFloat(el.data('count-interval')) : settings.interval; // Check direction
      ...
      countTimer = setInterval(function () {
        ...
        el.text(formatNumber(fromNumber)); // Check direction

        if (direction === 'up') fromNumber++;else fromNumber--;
      }, countInterval);
    }; // Clear counter
  ...
  $.fn.counter = function (options) {
    ...
    return this.each(function () {
      ...
      if (element.data('counter')) return; // Pass options

      var counter = new Counter(this, options); // Store plugin object in this element's data
  ...
  $.fn.counter.tmcOpts = {
    autoStart: true,
    // Boolean: Whether counter should start automatically
    from: 500,
    // Integer: starting number
    to: 100,
    // Integer: ending number
    interval: 20,
    // Integer: how often counter should update in milliseconds
...
/***/ "./src/vendor/scripts/jquery.tm.equalize.js":
/*!**************************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.equalize.js ***!
  \**************************************************/
/*! no static exports found */
...
(function ($) {
  ...
  var EqualizeHeights = function EqualizeHeights(element, options) {
    // Settings
    var settings = $.extend({}, $.fn.equalizeHeights.tmeOpts, options); // Object & jQuery el

    var el = $(element);
    var obj = this; // Startup variables
    ...
    obj.updateHeights = function () {
      ...
      if (settings.equalizeByGroup) {
        children.each(function () {
          ...
          columnY = $(this).position().top; // Clear row class & height

          $(this).attr('class', function (i, rowClass) {
          ...
          if (columnY !== rowY) {
            // Count rows created
            rowNumber++; // Reset target height

            targetHeight = 0; // Clear array
          ...
          columns.push($(this)); // Get target height

          targetHeight = targetHeight > $(this).outerHeight() ? targetHeight : $(this).outerHeight(); // Loop through add row class and set height

          $.each(columns, function (i) {
            columns[i].addClass('row-' + rowNumber).css({
        ...
        targetHeight = 0;
        targetHeight = el.outerHeight(); // Set heights
  ...
  $.fn.equalizeHeights = function (options) {
    ...
    return this.each(function () {
      ...
      if (element.data('equalizeHeights')) return; // Pass options

      var equalize = new EqualizeHeights(this, options); // Store plugin object in this element's data
  ...
  $.fn.equalizeHeights.tmeOpts = {
    equalizeByGroup: true,
    // Boolean: Whether all children within a given group shoulde be equalized on a per row basis
    updateOnResize: true,
    // Boolean: If heights should update on resize
...
/***/ "./src/vendor/scripts/jquery.tm.horizon.js":
/*!*************************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.horizon.js ***!
  \*************************************************/
/*! no static exports found */
...
(function ($, document, window, undefined) {
  ...
  var Horizon = function Horizon(element, options) {
    // Settings
    var settings = $.extend({}, $.fn.horizon.tmhOpts, options); // jQuery el

    element = $(element); // Mobile - set opacity and visibility
    ...
    element.data('scrolling', false).css('-webkit-backface-visibility', 'hidden'); // Element animation array
  ...
  */
  ...
  var requestScroll = function requestScroll(element, settings, startup) {
  ...
  */
  ...
  var updateElementState = function updateElementState(element, settings, startup) {
    if (isElementVisible(element, settings) || startup) {
      if (!settings.parallax) {
        ...
        var easing = element.data('e') ? easingArray[element.data('e')] : easingArray[settings.easing]; // Call animate
        ...
        if (mobile) return false; // Check direction, speed, rotation & opacity
        ...
        var opacity = element.data('po') ? element.data('po') : 'none'; // Calculate
        ...
        opacity = opacity === 'fade' ? distanceRemaining / element.parent().height() : 1; // Call animate
  ...
  */
  ...
  var isElementVisible = function isElementVisible(element, settings) {
    var winTop = $(window).scrollTop();
    var winBottom = winTop + $(window).height();
    ...
    var elTop = adjustedOffsetTop + element.outerHeight() * threshold; // Reposition only if fully outside of bounds
    // remove if resetting of state is required before

    if (offsetTop - winTop > $(window).height() || offsetTop - winTop < -element.outerHeight()) {
      if (settings.recurring) setElementState(element, settings); // Callback
  ...
  */
  ...
  var setElementState = function setElementState(element, settings) {
  ...
  */
  ...
  $.fn.horizon = function (options) {
    ...
    return this.each(function () {
      ...
      if (element.data('horizon')) return; // Pass options

      var horizon = new Horizon(this, options); // Store plugin object in this element's data
  ...
  $.fn.horizon.tmhOpts = {
    ...
    // Animation speed: milliseconds 
    threshold: 1,
    // Threshold: integer 0-1, how much of element should be visibel before animation begins
    recurring: true,
    // Recurring: boolean, if animation should occur over and over on scrolling
    parallax: false,
    // Parallax: boolean, whether element should parallax instead of animate
    parallaxSpeed: 0.2,
    // Parallax speed: integer 0.5, how fast parallax element should displace
    inView: null,
    // When element is in view and animation is done
...
/***/ "./src/vendor/scripts/jquery.tm.retinize.js":
/*!**************************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.retinize.js ***!
  \**************************************************/
/*! no static exports found */
...
(function ($) {
  ...
  $.fn.retinizeImages = function (options) {
    ...
    var settings = $.extend({
      retinaSupportMobile: false,
      // Whether swap should occur on mobile devices, optionally use [data-retina-mobile] for individual swaps
    ...
    */
    ...
    var replaceImage = function replaceImage() {
      ...
      if (mobile && !settings.retinaSupportMobile && !el.is('[data-retina-mobile]') || el.is('[data-no-retina]') || src.match(/\.(svg)/i) || src.indexOf(settings.retinaSuffix) >= 0) return false; // Swap if retina

      if (window.isRetinaDevice()) {
        // Swap
        var imgExt = src.substr(src.lastIndexOf('.'));
        src = src.replace(imgExt, settings.retinaSuffix + imgExt); // Load it before swap so there is no flickering
...
/***/ "./src/vendor/scripts/jquery.tm.snowbridge.js":
/*!****************************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.snowbridge.js ***!
  \****************************************************/
/*! no static exports found */
...
(function ($, document, window, undefined) {
  ...
  var SnowBridge = function SnowBridge(element, options) {
    // Settings
    var settings = $.extend({}, $.fn.snowBridge.tmpOpts, options); // jQuery el

    var container = $(element); // Startup data

    var refW = settings.scaleContainer ? settings.scaleUnder : container.width();
    container.data('animating', false).data('setup', false).data('refW', refW).data('refH', container.height()); // Size parallax container

    resizeParallax(container, settings); // Check fullscreen setting

    if (settings.fullscreen) settings.scaleContainer = true; // Check parallax factor

    if (settings.parallaxFactor > 1) settings.parallaxFactor = 1; // Setup done

    container.data('setup', true); // Preload
  ...
  */
  ...
  function preload(container, settings) {
    ...
    var parallaxEl = $('<div class="tm-parallax" />').prependTo($(container)); // Set target container

    if (mobile) parallaxEl.css({
      height: ''
    }).addClass('tmp-mobile'); // Add preloader to parllax element

    var loader = $('<div class="tm-loader"><div id="circle" /></div>');
    loader.appendTo(parallaxEl); // Check if retina
    ...
    $('<img class="tmp-media"/>').attr('src', src).one('load', function () {
      // Append image
      $(this).attr('src', src).appendTo(parallaxEl); // Remove loader

      parallaxEl.find('.tm-loader').remove(); // Call loaded
  ...
  */
  ...
  function loaded(container, settings) {
    // Media
    var media = container.find('.tmp-media'); // Set ref dimensions

    media.data('refW', media.width()).data('refH', media.height()); // Check opacity setting
    ...
    updateParallaxElement(container, settings); // Fade in
  ...
  */
  ...
  var requestScroll = function requestScroll(container, settings) {
  ...
  */
  ...
  var resizeParallax = function resizeParallax(container, settings) {
    // Window references
    var winW = $(window).width();
    var winH = $(window).height();
    var winR = winW / winH; // Container references
    ...
    var newContainerR = winW / containerNewH; // Resize container if 
    ...
    if (!container.data('setup')) return false; // Media references & size calculations
    ...
    var mediaNewW = mediaNewH * mediaR; // Scroll distance of container

    var scrollDistContainer = winH + containerNewH; // Center verticall if  mobile

    var topPos = !mobile ? 0 : -((mediaNewH - containerNewH) / 2); // Resize media
  ...
  */
  ...
  var updateParallaxElement = function updateParallaxElement(container, settings) {
    var media = container.find('.tmp-media');

    if (isElementVisible(container)) {
      if (mobile) return false; // Distance remaining until container is fully out of view up top

      var distanceRemaining = container.height() + container.offset().top - $(window).scrollTop(); // Current y position of container container

      var parallaxElTransY = container.offset().top - $(window).scrollTop(); // container y position of image
      // centered vertically

      var yPosImg = 0 - media.height() / 2; // Current y position of image

      var mediaTransY = yPosImg * (distanceRemaining / container.data('scrollDistContainer')) * settings.parallaxFactor; // Animate

      if (tSupport) {
        media.css({
          transform: 'translate3d( 0px' + ', ' + mediaTransY + 'px' + ', 0px)',
          visibility: 'visible'
        });
      } else {
        media.css({
      ...
      if (settings.fadeInOut) {
        ...
        media.parent().css({
          opacity: (Math.ceil(opacity * 100) / 100).toFixed(2)
        });
      }
    } else {
      if (!mobile) media.css({
  ...
  */
  ...
  var transform = prefix + '-transform';
  var transitionProperty = prefix + '-transition-property';
  var transitionDuration = prefix + '-transition-duration'; // Mobile Check
  ...
  $.fn.snowBridge = function (options) {
    ...
    return this.each(function () {
      ...
      if (element.data('snowBridge')) return; // Pass options

      var snowbridge = new SnowBridge(this, options); // Store plugin object in this element's data
  ...
  $.fn.snowBridge.tmpOpts = {
    fullscreen: false,
    // Whether parallax sections take screen dimensions or set dimensions
    parallaxFactor: 0.7,
    // Rate of parallax scrolling
    fadeInOut: true,
    // Fade parallax image in/out when entering/exiting viewport
    fadeThreshold: 0.5,
    // Threshold in pixels before fading starts/ends
    scaleContainer: true,
    // Whether parallax container should scale or not. If fullscreen, this defaults to true.
    scaleUnder: 1140,
    // Width under which slider should scale if fullwidth is true
    scaleMinHeight: 250,
    retinaSupport: true,
    // Check for retina displays and serve retina image
    retinaSupportMobile: false,
    // Whether swap should occur on mobile devices
    retinaSuffix: '@2x',
    // Retina image suffix
...
/***/ "./src/vendor/scripts/jquery.tm.summit.js":
/*!************************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.summit.js ***!
  \************************************************/
/*! no static exports found */
...
(function ($, document, window, undefined) {
  ...
  var SummitLightbox = function SummitLightbox(element, options) {
    // Settings
    var settings = $.extend({}, $.fn.summitLightbox.tmlOpts, options); // Object & jQuery el

    var link = $(element);
    var obj = this; // Gallery array

    var galleryGroup = []; // Link events

    link.on('click', function (event) {
      event.preventDefault(); // Check if link is part of group

      if ($(link).data('group')) galleryGroup = isGalleryGroup(link); // Link is now active

      $(this).addClass('tml-active'); // Lightbox Animation

      settings.lightboxAnimation = $(this).data('lightbox-animation') ? $(this).data('lightbox-animation') : settings.lightboxAnimation; // Lightbox Content Animation

      settings.contentAnimation = $(this).data('content-animation') ? $(this).data('content-animation') : settings.contentAnimation; // Build
    ...
    */
    ...
    obj.nextContent = function () {
      ...
      if (content.data('loading')) return false; // Remove caption of current
      ...
      removeContent(nextLink, 'next', settings); // Clear array
    ...
    obj.prevContent = function () {
      ...
      removeContent(prevLink, 'prev', settings); // Clear array
    ...
    */
    ...
    obj.destroyLightbox = function () {
      ...
      var content = $('#tml-content');
      var destroyTimer = null; // Remove any classes from body

      $('body').removeClass('modal-open'); // Get original position of lightbox

      var startPos = lightboxPosition(lightbox, settings);

      if (tSupport) {
        if (lightbox.data('transitioning')) return false; // Remove transition of children

        lightbox.children().css({
          transition: 'none'
        });
        content.css({
          transition: 'none'
      ...
      galleryGroup.length = 0; // Remove active

      $('.tml-active').removeClass('tml-active'); // Lighbox exit callback
  ...
  */
  ...
  var isGalleryGroup = function isGalleryGroup(link) {
    if ($(link).is('[data-group]')) {
      var group = $(link).data('group');
      var galleryGroup = []; // Check that all elements in the group have an ID, 
  ...
  */
  ...
  var lightboxPosition = function lightboxPosition(lightbox, settings) {
  ...
  */
  ...
  var buildLightbox = function buildLightbox(galleryGroup, settings, link, obj) {
    if ($('#tm-lightbox').length) return false; // Append lightbox

    $('body').append('<div id="tm-lightbox" class="tm-lightbox">' + '<div id="tml-content-wrapper" >' + '<div id="tml-content" />' + '</div>' + '<div id="tml-caption" />' + '</div>'); // References

    var lightbox = $('#tm-lightbox');
    var contentWrapper = $('#tml-content-wrapper');
    var content = $('#tml-content');
    var animateLbTimer = null; // Check whether modal mode
    ...
    lightbox.data('initLoad', true); // Check margin
    ...
    if (galleryGroup.length >= 2) {
      content.swipeIt({
        ...
        onSwipeMove: function onSwipeMove(position) {
          content.css({
            left: position + 'px'
          });
        },
    ...
    if (!settings.overlay) lightbox.css({
      background: 'none'
    }); // Add arrow nav
    ...
    if (settings.navArrows && galleryGroup.length >= 2) {
      ...
      var next = $('<a href="#" />').attr('id', 'tml-next').addClass('tml-nav').appendTo(lightbox); // Actions
    ...
    if (settings.navToolbar) {
      ...
      var popupUrl;
      var zoom;
      var share; // Build Toolbar

      var toolBar = $('<div id="tml-tool-bar" />').appendTo(lightbox); // Check settings
      ...
      settings.navShare = !link.is('[data-toolbar]') ? settings.navShare : $.inArray('share', toolbarArray) > -1 ? true : false; // Zoom

      if (settings.navZoom) {
        zoom = $('<a id="tml-zoom" class="tml-nav" href="#" title="Zoom"></a>').appendTo(toolBar); // Actions
      ...
      if (settings.navShare) {
        ...
        var listOrientation = link.is('[data-list-vertical]') ? 'list-vertical' : 'list-horizontal'; // Add share button and list

        share = $('<div id="tml-share-wrapper">' + '<a id="tml-share" class="tml-nav" href="#" title="Share"></a>' + '<ul class="tml-social-list ' + listOrientation + '" />' + '</div>').appendTo(toolBar); // Populate list
        ...
        $.each(networkArray, function (i, network) {
          share.find('ul').append('<li><a data-' + network + ' class="tml-nav" href="#">' + network + '</a></li>');
        }); // Actions
        ...
        share.find('ul a').on('click', function (event) {
          event.preventDefault(); // Get content

          var captionContent = link.data('caption');
          var twitterUser = link.data('twitter-user') ? link.data('twitter-user') : '';
          var imgUrl = link.data('image-url') ? link.data('image-url') : link.attr('href'); // Check network type
    ...
    if (tSupport) {
      lightbox.css({
        transition: 'none',
        transform: 'translate3d(' + startPos.x + 'px' + ', ' + startPos.y + 'px' + ', 0px)'
      });
  ...
  */
  ...
  var removeContent = function removeContent(link, direction, settings) {
    var lightbox = $('#tm-lightbox');
    var content = $('#tml-content');
    content.data('loading', true); // Check direction

    var tx = settings.contentAnimation === 'fade' ? 0 : settings.slideAmount;
    direction = direction === 'next' ? 1 : -1; // Call animate
  ...
  */
  ...
  var loadContent = function loadContent(link, direction, settings) {
    // Ref the lightbox
    var lightbox = $('#tm-lightbox');
    var content = $('#tml-content');
    var exit = $('#tml-exit').clone(true);
    var modalMode = lightbox.hasClass('tml-modal-mode') ? true : false; // IE9 fix for video audio
    // that keeps playing after removing content

    content.find('iframe').attr('src', ''); // Clear content		

    content.removeClass('tml-error scrollable-content').data('type', '').html(''); // Get url

    var href = link.attr('href'); // Match against image

    var img = href.match(/\.(jpeg|jpg|png|gif)/i); // Match against video

    var video = href.match(/(vimeo\.com|youtu(be\.com|\.be))\/(watch\?v=)?([A-Za-z0-9._%-]*)(\&\S+)?/); // Check if inline

    var inline = link.is('[data-content="inline"]'); // Check if iframe

    var iframe = link.is('[data-content="iframe"]'); // Check if ajax

    var ajax = link.is('[data-content="ajax"]'); // Add preloader

    if ($('#tml-loader').length) $('#tml-loader').remove();
    lightbox.append('<div id="tml-loader" class="tm-loader"><div id="circle" /></div>'); // Image
    ...
    if (img) {
      ...
      content.data('type', 'img'); // Load

      $('<img />').addClass('tml-image').attr('src', href).one('load', function () {
        content.html($(this));
    ...
    if (video) {
      // Set type data
      content.data('type', 'video'); // Create iframe
    ...
    if (inline) {
      // Set type data
      content.addClass('scrollable-content').data('type', 'inline'); // Check if inline el exists

      if ($(href).length) {
        var html = $(href).clone(true).addClass('show').css({
          visibility: 'visible',
          opacity: 1
        });
        content.html(html);
    ...
    if (iframe) {
      // Set type data
      content.addClass('scrollable-content').data('type', 'iframe'); // Create iframe
    ...
    if (ajax) {
      // Set type data
      content.addClass('scrollable-content').data('type', 'ajax'); // Find target container

      var tContainer = link.data('target-container'); // Load
  ...
  var loaded = function loaded(success, direction, settings) {
    var lightbox = $('#tm-lightbox');
    var content = $('#tml-content');
    var animateContentTimer = null; // Redimension content

    redimensionContent(settings); // Remove loader

    lightbox.find('#tml-loader').remove(); // Success
    ...
    if (success) {
      ...
      if (tSupport) {
        content.css({
          transition: 'none',
          opacity: 0,
          transform: 'translate3d(' + tx * direction + 'px' + ', 0, 0)'
        });
      ...
      processCaption(1, settings); // Lighbox content loaded callback

      if (settings.onLoaded) settings.onLoaded(); // Error 
    } else {
      var href = '';
      ...
      }).addClass('tml-error').html(settings.errorMessage + href);
  ...
  var redimensionContent = function redimensionContent(settings) {
    ...
    var modalH = lightbox.data('modal-height'); // Reference
    ...
    var contentH = 0;
    var top = 0;
    var left = 0; // Clear previous

    content.not('.tml-error').css({
      height: 'auto',
      width: 'auto',
      top: '',
      left: ''
    ...
    top = wrapperH > contentH ? (wrapperH - contentH) / 2 : 0; // Set
  ...
  */
  ...
  var animate = function animate(selector, opacity, transX, transY, settings) {
    var lightbox = $('#tm-lightbox');
    var content = $('#tml-content'); // Reset data to false for content

    if (selector.attr('id') === 'tm-lightbox') lightbox.data('transitioning', true); // Animation magic
    ...
    attrs.visibility = 'visible';
    attrs.opacity = opacity;
    selector.css(attrs).one(transitionEnd, function (event) {
      // Prevent bubbling
      event.stopPropagation(); // Remove listener

      $(this).off(transitionEnd); // Reset data to false for content
  ...
  */
  ...
  var processCaption = function processCaption(opacity, settings) {
  ...
  */
  ...
  $.fn.summitLightbox = function (options) {
    ...
    return this.each(function () {
      ...
      if (element.data('summitLightbox')) return; // Pass options

      var summit = new SummitLightbox(this, options); // Store plugin object in this element's data
  ...
  $.fn.summitLightbox.tmlOpts = {
    ...
    // Animation type: fade, slideInTop, slideInRight, slideInBottom, slideInLeft
    contentAnimation: 'slide',
    // Animation type: Slide or fade
    slideAmount: 100,
    // Animation amount: Amount to slide on x axis
    easing: 'swing',
    // Easing type: string, see easingArray
    speed: 350,
    // Animation speed: milliseconds, lowest 50 in which no animation occurrs
    // Overlay
    overlay: true,
    // Overlay: true or false
    // Width/Margin
    maxWidth: 1140,
    // Max width for video, inline, iframe and ajax content
    contentMargin: 15,
    // Content margin 15 is 15% of window width and height
    // Navigation
    navArrows: true,
    // Arrow nav: boolean
    navKeyboard: true,
    ...
    // Exit nav: boolean
    navToolbar: true,
    // Tool bar with share and zoom: boolean
    navZoom: true,
    // Zoom nav: boolean
    navShare: true,
    // Social nav: boolean
    navNetworks: 'facebook,twitter,pinterest',
    // Networks to exclude - list as facebook,twitter etc.
    overlayClickClose: true,
    // Close lightbox upon clicking overlay: boolean
    // Retina Support
    retinaSupport: false,
    // Check for retina displays and serve retina image
    retinaSupportMobile: false,
    ...
    // Error message
    showErrorSrc: true,
    // Show the src causing the issue: boolean
    // Callback
    onSetup: null,
    // Callback: when lightbox is created
    onLoaded: null,
    // Callback: after requested content has loaded
...
/***/ "./src/vendor/scripts/jquery.tm.swipe.js":
/*!***********************************************!*\
  !*** ./src/vendor/scripts/jquery.tm.swipe.js ***!
  \***********************************************/
/*! no static exports found */
...
(function ($) {
  ...
  $.fn.swipeIt = function (options) {
    ...
    var settings = $.extend({
      swipeThreshold: 40,
      // Amount to be swiped in px before action occurrs
      scrollThreshold: 10,
      draggable: false,
      // Whether element should be draggable before release
      preventTouchOn: '',
      // Class or id that should not receive touch
      onSwipeMove: null,
      // Callback: when user is moving
    ...
    var touchStop = hasTouch ? 'touchend' : 'pointermove'; // Start

    var swipeStart = function swipeStart(event) {
      event.stopPropagation();
    ...
    var swipeMove = function swipeMove(event) {
...
$(document).ready(function () {
  ...
  */
  ...
  var sideNavigation = {
    init: function init() {
      if (!$(sideNav).length) return false; // Nav active

      $('body').data('aux-nav', false); // Determine animation type & scrollbar setting

      var animation = $(sideNav).data('animation') ? $(sideNav).data('animation') : 'no-transition';
      var scrollbar = $(sideNav).is('[data-no-scrollbar]') ? 'no-scrollbar' : 'scrollbar'; // Add reveal and reset classes

      if (animation === 'no-transition') $(siteWrapper).addClass(animation + '-reset');
      $(siteWrapper).addClass('reveal-side-navigation');
      $(sideNav).addClass(animation + '-reset ' + scrollbar); // Determine wrapper animation direction 
      ...
      sideNavHelperClass = sideNavHelperClass.split('.').join(''); // Toggle buttons and actions
      ...
      $(sideNavShow).on(eventType, function (event) {
        ...
        if (!$(sideNav).hasClass('active')) {
          ...
          auxNavigation.state(true); // Nav now active add helper class to body

          $('body').data('aux-nav', true);
          $('html, body').addClass(sideNavHelperClass); // Lock Header

          var scrollPos = $(window).scrollTop();
          if (!mobile) $('.header-inner').css({
            top: scrollPos + 'px'
          }); // Add animation & state classes

          if (tSupport) {
            $(siteWrapper).addClass(direction + ' inactive ' + animation).css({
              transitionTimingFunction: easingArray[cntEasing]
            });
            $(sideNav).addClass('active ' + animation).css({
              transitionTimingFunction: easingArray[cntEasing]
            });
          } else {
          ...
          $(siteWrapper).on(transitionEnd, function (event) {
            ...
            event.stopPropagation();
            $(this).off(transitionEnd); // Add overlay click event
    ...
    closeNav: function closeNav(direction, animation) {
      if (tSupport) {
        // Reenable transitions
        transitions.state(true); // Remove direction & animation classes

        $(siteWrapper).removeClass(direction + ' ' + animation);
        $(sideNav).removeClass(animation); // Remove classes directly if
        // no animation is set
  ...
  */
  ...
  var overlayNavigation = {
    init: function init() {
      if (!$(overlayNav).length) return false; // Determine animation type & scrollbar setting

      var animation = $(overlayNav).data('animation') ? $(overlayNav).data('animation') : 'no-transition';
      var scrollbar = $(overlayNav).is('[data-no-scrollbar]') ? 'no-scrollbar' : 'scrollbar'; // Add reveal and reset classes

      $(overlayNav).addClass(animation + '-reset ' + scrollbar); // Strip .

      overlayNavHelperClass = overlayNavHelperClass.split('.').join(''); // Toggle button actions
      ...
      $(overlayNavShow).on(eventType, function (event) {
        ...
        if (!$(overlayNav).hasClass('active')) {
          // Nav active
          $('body').data('aux-nav', true);
          $('html, body').addClass(overlayNavHelperClass); // Add classes

          if (tSupport) {
            $(overlayNav).addClass('active ' + animation).css({
              transitionTimingFunction: easingArray[overlayEasing]
            });
          } else {
    ...
    closeNav: function closeNav(animation) {
      if (tSupport) {
        $(overlayNav).removeClass(animation); // Remove classes directly if
        // no animation is set
        ...
        $(overlayNav).removeClass('active').css({
          top: '-100%'
        });
      } // Nav inactive
    ...
    init: function init() {
      ...
      $(menuContainer).find(subMenuParentLink).each(function () {
        $(this).on(eventType, function (event) {
          event.preventDefault(); // Swap active of parent
          ...
          transitions.state(true); // Loop through and add 
          ...
          if ($(this).siblings(subMenu).hasClass('open')) {
            $(this).closest(menuContainer).find(subMenu).css({
              height: 0
            }).removeClass('open');
          } else {
            $(this).closest(menuContainer).find(subMenu).css({
  ...
  */
  ...
  var thresHold = 300; // Scroll to top button
  // class ref

  var toTopButton = '.scroll-to-top'; // Scroll to top speed
  ...
  */
  ...
  var header = '.header'; // Tab class

  var _tabs = '.tabs';
  var tabTriggerClick = '.tab-trigger-click'; // Accodion class and 
  // open/close icon ref
  ...
  var accTriggerClick = '.accordion-trigger-click'; // Dismissable Element
  // and fadeout speed

  var _dismissable = '.box.dismissable';
  var dismSpeed = 300;
  var dismEasing = 'swing'; // Dropdown element
  // and fadeout speed

  var _dropdown = '.dropdown';
  var ddSpeed = 300;
  var ddEasing = 'swing'; // Thumb and overlay class
    ...
    tabs: function tabs() {
      ...
      $(tabTriggerClick).each(function () {
        $(this).on('click', function () {
          ...
          $(_tabs).find(targetTab).trigger('click');
          return false;
        });
      });
    },
      ...
      $(accordion).each(function () {
        ...
        accLink.each(function () {
          ...
          $(this).on(eventType, function () {
            ...
            var allPanels = link.closest(accordion).find('li > div'); // Set height of all active
            ...
            if (link.parent().hasClass('active')) {
              toggleTimer = setTimeout(function () {
                if (!link.closest(accordion).is('[data-toggle-multiple]')) {
                  link.closest(accordion).find('li.active > div').removeClass('no-transition').css({
                    height: 0
                  });
                } else {
                  link.siblings(targetPanel).removeClass('no-transition').css({
                    height: 0
                  });
                }
              ...
              toggleTimer = setTimeout(function () {
                ...
                if (!link.closest(accordion).is('[data-toggle-multiple]')) {
                  allPanels.removeClass('no-transition').css({
                    height: 0
                  });
        ...
        $(this).find('.close').on(eventType, function (event) {
          ...
          if (tSupport) {
            $(this).parent().css({
              transitionProperty: 'opacity',
              opacity: 0,
              transitionDuration: dismSpeed + 'ms',
    ...
    dropdown: function dropdown() {
      $(_dropdown).each(function () {
        $(this).children('.button, button').not().each(function () {
          $(this).on(eventType, function (event) {
            event.preventDefault();
            $(_dropdown).children('.button, button').removeClass('active');
            ...
            $(document).on('click.closeDropdown', function (event) {
              if (!$(event.target).closest(_dropdown).length) {
                closeAllDropdowns();
              }
            });
          });
    ...
    rollovers: function rollovers() {
      if (tSupport) {
        $(thumb).each(function () {
          ...
          var opacity = $(this).data('hover-bkg-opacity') ? $(this).data('hover-bkg-opacity') : 1; // Check for backgorund color

          if ($(this).data('hover-bkg-color')) {
            var bkg = $(this).data('hover-bkg-color');
            bkg = bkg.replace('#', ''); // Convert hex to rgba
          ...
          $(this).find(tmbAnimationEl).css({
            transitionDuration: speed + 'ms',
            transitionTimingFunction: easingArray[easing]
          });
          $(this).find(tmbOverlay).css({
            backgroundColor: rgba
          });
        });
      } else {
        $(thumb).find('.overlay-link').mouseenter(function () {
          ...
          $(this).find(tmbOverlay).stop().animate({
            opacity: 0
          }, ovlSpeed, ovlEasing);
        });
      }
    },
    thumbnailRatio: function thumbnailRatio() {
      $(window).on('resize', function () {
        $(thumb).each(function () {
  ...
  */
  ...
  if (tSupport) document.getElementsByTagName('body')[0].className += ' transition-support'; // Easing Array
  // For converting jQuery easing types
  // to Cubic Bezier format for CSS3 transitons
  ...
  if (isWebkit) document.getElementsByTagName('body')[0].className += ' webkit'; // Safari Hack

  var isSafari = /constructor/i.test(window.HTMLElement);
  if (isSafari) document.getElementsByTagName('body')[0].className += ' safari-browser'; // IE Hack
